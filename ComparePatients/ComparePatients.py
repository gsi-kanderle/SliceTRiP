	  ...
	  if voi:
	    ...
	    if voiName.find('lung') > -1:
	      ...
	      if voiIpsiLateral and voiContraLateral:
	        output_str += str(getattr(voiIpsiLateral,metric)/voiIpsiLateral.perscDose) +'\t'
	        output_str += str(getattr(voiContraLateral,metric)/voiContraLateral.perscDose) +'\t'
	        voiIpsiLateral = None
	        voiContraLateral = None
	        firstLung = False
	    else:
	      output_str += str(getattr(voi,metric)/voi.perscDose) +'\t'
	  else:
	    output_str += '\t'
	output_str += '\n'
      else:
